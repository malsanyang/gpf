...
interface CriminalIndexPageProps {
    data: Array<CriminalListProps>,
    currentUser: { data: UserListProps } | null,
}

const Index = ({ data, currentUser } : CriminalIndexPageProps) => {
    return (
        <Layout currentUser={currentUser?.data}>
            <Head title={'Criminal List'} />

            <Breadcrumb paths={['criminals']} />

            <Link
                href={route('criminals.create', undefined, undefined, undefined)}
            ...
            <div className="w-full max-w-full rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                <div className="max-w-full overflow-x-auto">
                    <table className="w-full table-auto">
                        ...
                        { data.map(function (criminal, index){
                            return (
                                <tr key={index}>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{criminal.fullName}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{criminal.address}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{criminal.telephoneNo}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <p className="text-black dark:text-white">{new Date(criminal.dob).toLocaleDateString()}</p>
                                    </td>
                                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                        <div className="flex items-center space-x-3.5">
                                            <Link className="hover:text-primary" href={route('criminals.show', criminal.id, undefined, undefined)}>
                                                <svg
                                            ...
                                            <Link className="hover:text-primary"
                                                  href={route('criminals.edit', criminal.id, undefined, undefined)}>
                                                <svg
                                                    className="fill-current"
                                                    width="18"
                                                    height="18"
                                            ...
                                            <Link className="hover:text-primary"
                                                  href={route('criminals.delete', criminal.id, undefined, undefined)}
                                                  method="delete"
                                                  as="button"
                                                  type="button">
                                                <svg
